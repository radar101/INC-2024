...
class ArweaveGatewayBundledContractDefinitionLoader {
    ...
    async load(contractTxId, evolvedSrcTxId) {
        ...
        const contractTx = await this.fetchContractTx(contractTxId);
    ...
    async fetchContractTx(contractTxId) {
    ...
    async contractSource(contractSrcTx, contractType) {
        const uploaderId = getTagValue(contractSrcTx.tags, KnownTags_1.WARP_TAGS.UPLOADER_TX_ID);
        if (uploaderId != null) {
            ...
            return arweave_1.default.utils.b64UrlToString(JSON.parse(txString).data);
        }
