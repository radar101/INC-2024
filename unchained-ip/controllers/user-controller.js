...
const userProfileUpdate = async (req, res) => {
  ...
  await User.findOneAndUpdate(
    { _id: userId },
    { $set: { name, email, phone, created_at: new Date() } }
  )
    .then((user) => {
      console.log(user);
...
const createIpDocument = async (req, res) => {
  try {
    ...
    const id = await blockchain.addIpRecordToContract(
      newTitle,
      newIpType,
      newProofs,
      newDescription,
...
const createWillDocument = async (req, res) => {
  try {
    ...
    const newExecutorDigitalSign = await uploadFileOnIpfs(
      useExecutorDigitalSign
    );

    const useTestatorDigitalSign = req.files[2].originalname;
    const newTestatorDigitalSign = await uploadFileOnIpfs(
      useTestatorDigitalSign
    );
    ...
    const id = await blockchain.addWillRecordToContract(
      newExecutorName,
      newExecutorIdProof,
